...
- (bool)handleKeyEvent:(NSEvent *)nsevent
{
    ...
    if (keyEvent.nativeModifiers & NSEventModifierFlagNumericPad) {
    ...
    if ([charactersIgnoringAllModifiers length] != 0) {
        ...
        // We apparently need to use the same lookup to get the key code for the
        // noModCh obtained from NS_Event::charactersByApplyingModifiers() as we
        // do in the legacy sidefxLookupKeyString() fallback rather than the one
        // used for NS_Event::characters() or charactersIgnoringModifiers() in
        // the regular codepath.
        // NB: The noModText will likely be different for non-symbol keys from
        //     text in the regular codepath even when no modifiers are held.
        //noModKeyCode = [self convertKeyCode:noModCh];
        noModKeyCode = QCocoaIntegration::instance()->sidefxLookupKeyStringCharToQtKey(noModCh, keyEvent.nativeVirtualKey, Qt::NoModifier);
    }
    QString noModText;
    ...
    QString charactersIgnoringAllModifiers = QCocoaIntegration::instance()->sidefxLookupKeyString(keyEvent.nativeVirtualKey, Qt::NoModifier);
    if (charactersIgnoringAllModifiers.length() != 0) {
        noModCh = charactersIgnoringAllModifiers.at(0);
        // We need to use a different conversion to the key code for the noModCh
        // obtained from sidefxLookupKeyString() as non-symbol keys are not
        // encoded the same way as by NS_Event::characters(). Bizzarely we have
        // the same problem with NS_Event::charactersByApplyingModifiers() in
        // the newer codepath above.
        // NB: The noModText will likely be different for non-symbol keys from
        //     text in the regular codepath even when no modifiers are held.
        noModKeyCode = QCocoaIntegration::instance()->sidefxLookupKeyStringCharToQtKey(noModCh, keyEvent.nativeVirtualKey, Qt::NoModifier);
    }
    ...
    if (m_sendKeyEvent && m_composingText.isEmpty()) {
		...
		window, keyEvent.timestamp, preKeyEventType, noModKeyCode,
		noModModifiers, keyEvent.nativeScanCode,
		keyEvent.nativeVirtualKey, noModNativeModifiers, noModText,
		[nsevent isARepeat], 1);
        }
        ...
        if (postKeyEventType != QEvent::None) {
            QWindowSystemInterface::handleExtendedKeyEvent(
		window, keyEvent.timestamp, postKeyEventType, noModKeyCode,
		noModModifiers, keyEvent.nativeScanCode,
		keyEvent.nativeVirtualKey, noModNativeModifiers, noModText,
		[nsevent isARepeat], 1);
        }
