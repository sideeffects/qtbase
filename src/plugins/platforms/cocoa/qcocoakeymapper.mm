...
int QCocoaKeyMapper::sidefxLookupKeyStringCharToQtKey(
                                                const QChar &ch,
                                                unsigned short macVirtualKey,
                                                Qt::KeyboardModifiers modifiers)
{
    // TODO: qt_mac_get_key() checks for Qt::ShiftModifier, but the way it's
    ...
    const UInt32 keyModifier = ((qt_mac_get_mac_modifiers(modifiers) >> 8) & 0xFF);
    int qtkey = qt_mac_get_key(keyModifier, ch, macVirtualKey);
    if (qtkey == Qt::Key_unknown)
        qtkey = ch.unicode();
